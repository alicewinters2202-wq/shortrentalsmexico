...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ city?: string }> }) {
  ...
  return { alternates: { canonical: city ? `https://shortstaymx.com/properties?city=${encodeURIComponent(city)}` : 'https://shortstaymx.com/properties' }, title, description, openGraph: { title, description, url: city ? `https://shortstaymx.com/properties?city=${encodeURIComponent(city)}` : 'https://shortstaymx.com/properties' } };
}
