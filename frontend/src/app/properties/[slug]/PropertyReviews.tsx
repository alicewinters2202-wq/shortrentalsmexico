...
export default function PropertyReviews({ slug, initialReviews, lang }: Props) {
  const [reviews] = useState<PropertyReview[]>(initialReviews);
  ...
  return (
    <div>
        ...
        <div>
          ...
          {avg ? (
            ...
            <p className="text-sm" style={{ color: 'var(--muted)' }}>
              {lang === 'en' ? 'No reviews yet.' : 'Aún no hay reseñas.'}
            </p>
          )}
        </div>
        {!showForm && !submitted && (
      ...
      {submitted && (
        <div className="rounded-2xl p-6 mb-6 text-center" style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}>
          <p className="font-serif text-lg mb-1" style={{ color: 'var(--ink)' }}>
            {lang === 'en' ? 'Thank you for your review' : 'Gracias por tu reseña'}
          </p>
          <p className="text-sm" style={{ color: 'var(--muted)' }}>
            {lang === 'en' ? "It's been received and will appear here once reviewed." : 'Fue recibida y aparecerá aquí una vez revisada.'}
          </p>
        </div>
      )}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-2xl p-6 mb-6" style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}>
          ...
          <p className="text-xs mb-3" style={{ color: 'var(--muted)' }}>
            {lang === 'en' ? 'Reviews are reviewed before they appear publicly.' : 'Las reseñas se revisan antes de publicarse.'}
          </p>
