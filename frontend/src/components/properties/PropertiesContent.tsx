...
import SortDropdown from '@/app/properties/SortDropdown';
import PriceRangeFilter from '@/app/properties/PriceRangeFilter';
import BedroomsFilter from '@/app/properties/BedroomsFilter';
import ViewToggle from '@/app/properties/ViewToggle';
import PropertiesMap from '@/app/properties/PropertiesMap';
import SaveButton from '@/app/properties/SaveButton';
import SavedLink from '@/app/properties/SavedLink';
import CompareButton from '@/app/properties/CompareButton';
import CompareBar from '@/app/properties/CompareBar';
...
export default async function PropertiesContent({
  lang,
  searchParams,
}: {
  lang: 'es' | 'en';
  ...
  const basePath = lang === 'en' ? '/en/properties' : '/properties';
          ...
          <div className="mb-7">
            {guestsParam !== undefined && hiddenByFilterCount > 0 && (
              <div className="flex items-center gap-2 flex-wrap text-sm rounded-xl px-4 py-3" style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}>
                ...
                <Link
                  href={(() => { const p = new URLSearchParams(); if (cityParam) p.set('city', cityParam); if (sortParam) p.set('sort', sortParam); const qs = p.toString(); return `${basePath}${qs ? `?${qs}` : ''}`; })()}
