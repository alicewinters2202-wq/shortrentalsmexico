...
export default function AgentsContent({ lang }: { lang: 'es' | 'en' }) {
  ...
  const agentsPath = lang === 'en' ? '/en/agents' : '/agents';
    ...
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      ...
      <nav className="sticky top-0 z-40 backdrop-blur-md"
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href={homePath} className="font-serif font-medium tracking-wide" style={{ color: 'var(--ink)' }}>
            ShortStayMX
          </Link>
          <div className="flex items-center gap-6">
            <Link href={propertiesPath} className="text-xs hover:opacity-80" style={{ color: 'var(--muted)' }}>
              {t.allProperties}
            </Link>
          ...
          {AGENTS.filter((a) => !a.specialist).map((agent) => {
            ...
            return (
              <div key={agent.id} className="rounded-3xl p-8 flex flex-col items-center gap-5"
                ...
                <Link
                  href={`${propertiesPath}?city=${encodeURIComponent(city)}`}
      ...
      <footer style={{ borderTop: '1px solid var(--border)' }} className="py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-5 text-xs" style={{ color: 'var(--muted)' }}>
            <Link href={propertiesPath} className="hover:opacity-80">{t.allProperties}</Link>
            <Link href={aboutPath} className="hover:opacity-80">{t.aboutNav}</Link>
            <Link href={agentsPath} className="hover:opacity-80">{t.agentsSectionTitle}</Link>
          </div>
        </div>
      </footer>
    </div>
