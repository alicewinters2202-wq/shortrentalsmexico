...
import CompareImageBrowser from '@/app/properties/compare/CompareImageBrowser';
import BackLink from '@/app/properties/compare/BackLink';

export default async function CompareContent({
  lang,
  searchParams,
}: {
  lang: 'es' | 'en';
    ...
    return (
      <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }} className="flex items-center justify-center px-6">
        <div className="text-center max-w-md">
          ...
          <Link href={propertiesPath} className="text-sm font-semibold underline" style={{ color: 'var(--ochre)' }}>
  ...
  return (
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      <div className="max-w-6xl mx-auto px-6 py-10">
        <BackLink fallbackHref={propertiesPath} label={lang === 'en' ? '← Back to properties' : '← Volver a propiedades'} />
                ...
                {selected.map((p) => {
                  ...
                  return (
                    <th key={p.id} className="p-4 text-left align-top">
                      ...
                      <Link href={`${lang === 'en' ? '/en' : ''}/properties/${p.slug}`} className="block hover-float">
