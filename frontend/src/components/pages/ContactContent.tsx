...
export default function ContactContent({ lang }: { lang: 'es' | 'en' }) {
    ...
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      <nav className="sticky top-0 z-40 backdrop-blur-md" style={{ backgroundColor: 'rgba(28,28,30,0.9)', borderBottom: '1px solid var(--border)' }}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
          <Link href={lang === 'en' ? '/en' : '/'} className="text-sm transition-colors hover:opacity-80" style={{ color: 'var(--muted)' }}>← {lang === 'en' ? 'Home' : 'Inicio'}</Link>
          <span style={{ color: 'var(--border)' }}>|</span>
          <Link href={lang === 'en' ? '/en' : '/'} className="font-serif font-medium" style={{ color: 'var(--ink)' }}>ShortStayMX</Link>
