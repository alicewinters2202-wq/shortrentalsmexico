...
export default function ComoFuncionaContent({ lang }: { lang: 'es' | 'en' }) {
  const es = lang !== 'en';
  const homePath = lang === 'en' ? '/en' : '/';
  const propertiesPath = lang === 'en' ? '/en/properties' : '/properties';
  ...
  const faqs = [
    ...
    },
    {
      ...
      a_es: 'Todas nuestras propiedades incluyen mobiliario completo, WiFi de alta velocidad, agua caliente, electricidad, gas y acceso a las amenidades del edificio. Sin impuestos ni cargos de servicios ocultos — el precio que ves es el precio que pagas por la renta.',
      a_en: 'All our properties include full furniture, high-speed WiFi, hot water, electricity, gas and access to building amenities. No hidden taxes or utility charges — the price you see is the price you pay for rent.',
    },
    {
    ...
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      <nav className="sticky top-0 z-40 backdrop-blur-md" style={{ backgroundColor: 'rgba(28,28,30,0.9)', borderBottom: '1px solid var(--border)' }}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
          <Link href={homePath} className="text-sm transition-colors hover:opacity-80" style={{ color: 'var(--muted)' }}>← Home</Link>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="mb-16">
          ...
          <h1 className="italic text-5xl sm:text-6xl mb-6" style={{ color: 'var(--ink)', fontFamily: 'var(--font-display), serif' }}>
        ...
        <div className="rounded-2xl p-10 text-center" style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}>
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <Link href={propertiesPath} className="inline-block px-8 py-3 rounded-full text-sm font-medium text-white" style={{ backgroundColor: 'var(--gold)' }}>
