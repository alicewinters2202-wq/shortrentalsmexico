...
export default function RequirementsContent({ lang }: { lang: 'es' | 'en' }) {
  const t = T[lang] as unknown as TType;
  const propertiesPath = lang === 'en' ? '/en/properties' : '/properties';
  const aboutPath = lang === 'en' ? '/en/about' : '/about';
  const reqPath = lang === 'en' ? '/en/requirements' : '/requirements';
    ...
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href={lang === 'en' ? '/en' : '/'} className="font-serif font-medium tracking-wide" style={{ color: 'var(--ink)' }}>
            ShortStayMX
          </Link>
          <div className="flex items-center gap-6">
            <Link href={propertiesPath} className="text-xs transition-colors hover:opacity-80" style={{ color: 'var(--muted)' }}>
              {t.allProperties}
            </Link>
            <Link href={aboutPath} className="text-xs transition-colors hover:opacity-80" style={{ color: 'var(--muted)' }}>
              {t.aboutNav}
            </Link>
      ...
      <footer style={{ borderTop: '1px solid var(--border)' }} className="py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-5 text-xs" style={{ color: 'var(--muted)' }}>
            <Link href={propertiesPath} className="hover:opacity-80 transition-opacity">{t.allProperties}</Link>
            <Link href={aboutPath} className="hover:opacity-80 transition-opacity">{t.aboutNav}</Link>
            <Link href={reqPath} className="hover:opacity-80 transition-opacity">{t.reqNav}</Link>
          </div>
        </div>
      </footer>
    </div>
