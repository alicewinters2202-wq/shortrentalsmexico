...
export default function ColoniasContent({ lang }: { lang: "es" | "en" }) {
  const es = lang !== "en";
  const homePath = lang === "en" ? "/en" : "/";
  const coloniasPath = (slug: string) => lang === "en" ? `/en/colonias/${slug}` : `/colonias/${slug}`;
  return (
            ...
            <div key={city.city_es}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {city.colonias.map(col => (
                  <Link key={col.slug} href={coloniasPath(col.slug)} className="group block">
