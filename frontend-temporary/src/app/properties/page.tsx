...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ city?: string }> }) {
  ...
  return { alternates: { canonical: city ? `https://temporaryrentalsmexico.com/properties?city=${encodeURIComponent(city)}` : 'https://temporaryrentalsmexico.com/properties' }, title, description, openGraph: { title, description, url: city ? `https://temporaryrentalsmexico.com/properties?city=${encodeURIComponent(city)}` : 'https://temporaryrentalsmexico.com/properties' } };
}
...
export default async function PropertiesPage({
  ...
  return (
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-10">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <BedroomsFilter
              ...
              accentColor="var(--gold)"
            />
            <SavedLink active={idsParam !== undefined} lang={lang} accentColor="var(--gold)" />
          ...
          <PriceRangeFilter
            ...
            guestsParam={guestsParam}
            sortParam={sortParam}
            minBound={priceMinBound}
            maxBound={priceMaxBound}
        ...
        {viewParam === 'map' ? (
          ...
          <div className="text-center py-20">
            ...
            <Link href="/properties" className="text-sm font-semibold underline" style={{ color: 'var(--gold)' }}>
              ...
              <Link key={p.id} href={`/properties/${p.slug}`} className="group block hover-float">
                <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-lg shadow-black/30" style={{ backgroundColor: 'var(--card)' }}>
                  ...
                  <div className="absolute top-3 left-3 flex flex-col items-start gap-1.5">
                    ...
                    <CompareButton propertyId={p.id} lang={lang} accentColor="var(--gold)" />
                  </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex gap-1 mt-2 flex-wrap">
                    ...
                    {p.selfCheckIn && <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ backgroundColor: "rgba(63,184,175,0.15)", color: "var(--gold)" }}>🔑 {t.selfCheckIn}</span>}
                  </div>
                </div>
              </Link>
            );
      ...
      <CompareBar lang={lang} accentColor="var(--gold)" />
    </div>
  );
}