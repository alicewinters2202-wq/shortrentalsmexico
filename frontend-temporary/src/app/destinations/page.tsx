...
export async function generateMetadata() {
  return {
    title: 'Destinos | TemporaryRentalsMexico',
    description: 'Explora todas las ciudades donde TemporaryRentalsMexico tiene propiedades disponibles para renta temporal en México.',
  };
}
...
export default async function DestinationsPage() {
  ...
  return (
    <div style={{ backgroundColor: 'var(--cream)', minHeight: '100vh' }}>
      <nav className="sticky top-0 z-40 backdrop-blur-md" style={{ backgroundColor: 'rgba(16,24,32,0.9)', borderBottom: '1px solid var(--border)' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-14">
        <p className="text-[11px] tracking-[0.3em] uppercase font-medium mb-3" style={{ color: 'var(--gold)' }}>
          {lang === 'en' ? 'Explore' : 'Explora'}
        </p>
        <h1 className="font-serif mb-3 leading-[0.95] text-5xl sm:text-6xl" style={{ color: 'var(--ink)' }}>
          ...
          {ALL_CITIES.map((c) => {
            ...
            return (
              <Link key={c.name} href={`/properties?city=${encodeURIComponent(c.name)}`}
                ...
                <div className="absolute top-3 right-3 text-[10px] font-semibold px-2.5 py-1 rounded-full" style={{ backgroundColor: 'var(--gold)', color: 'var(--cream)' }}>
                  {t.properties(count)}
                </div>
